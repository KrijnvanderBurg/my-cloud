...
locals {
  ...
  location       = "westeurope"
  location_short = "weu"
  location_index = 1 # weu=1, gwc=2, ..., etc.
  common_tags = {
  ...
  hub_managed_subnets = {
    ...
    "snet-nva-co-${local.environment}-${local.location_short}-01"               = cidrsubnet(local.hub_cidr, 7, 14) # 10.1.1.192/27  (32 IPs) - NVA
    "snet-dns-inbound-co-${local.environment}-${local.location_short}-01"       = cidrsubnet(local.hub_cidr, 7, 15) # 10.1.1.224/27  (32 IPs) - DNS inbound
    "snet-dns-outbound-co-${local.environment}-${local.location_short}-01"      = cidrsubnet(local.hub_cidr, 7, 16) # 10.1.2.0/27    (32 IPs) - DNS outbound
    "snet-management-co-${local.environment}-${local.location_short}-01"        = cidrsubnet(local.hub_cidr, 7, 17) # 10.1.2.32/27   (32 IPs) - Jump boxes
    "snet-private-endpoints-co-${local.environment}-${local.location_short}-01" = cidrsubnet(local.hub_cidr, 6, 9)  # 10.1.2.64/26   (64 IPs) - Private endpoints
    "snet-shared-services-co-${local.environment}-${local.location_short}-01"   = cidrsubnet(local.hub_cidr, 6, 10) # 10.1.2.128/26  (64 IPs) - Shared services
  }
}
