#!/bin/bash
plan_binary_file="${1:?Plan binary file path is required}"
plan_text_file="${2:?Plan text file path is required}"

tofu plan -destroy -no-color -out="$plan_binary_file" 2>&1 | tee "$plan_text_file"
